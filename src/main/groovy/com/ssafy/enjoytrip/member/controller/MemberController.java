...
public class MemberController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<MyInfo> selectById(@PathVariable("id") String id) {
        System.out.println("검색 요청 아이디 : " + id);
//        MemberDto memberDto = memberService.selectById(id);
//        System.out.println("검색된 정보 : " + memberDto);
        MyInfo myInfo = memberService.selectById(id);
        System.out.println("응답완료 : " + myInfo);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(myInfo);
    }
    ...
    public ResponseEntity<?> updateUser(@RequestBody MemberDto member) {
        ...
        MemberDto original = memberService.selectUser(member.getId());
        System.out.println("기존 유저 정보 : " + member);
        if (member.getNickname() != null) original.setNickname(member.getNickname());
        memberService.updateUser(original);
        return ResponseEntity
                .status(HttpStatus.OK)
                .build();
    }
